...
impl BinanceClient {
    ...
    pub async fn fetch_latest_quotes(
        ...
        let url = format!(
            "https://api.binance.com/api/v3/ticker/bookTicker?symbols={}",
            symbols_json
        );
    ...
    pub async fn place_order(
        ...
        let query = format!(
            "symbol={}&side={}&type=MARKET&quantity={:.5}&recvWindow=60000&timestamp={}",
            symbol, side_str, qty, timestamp
        );
        ...
        println!(
            "Successfully submitted {} order for {} to Binance Testnet API",
            side_str, symbol
        );
        Ok(())
    }
