...
pub async fn start_alpaca_ws(
    ...
    match connect_async(url).await {
        Ok((mut ws_stream, _)) => {
            ...
            let _ = ws_stream.send(Message::Text(auth_msg.to_string().into())).await;
                        ...
                        if let Some(arr) = data.as_array() {
                            for item in arr {
                                if let Some(msg_type) = item.get("T").and_then(|v| v.as_str()) {
                                    if msg_type == "q" {
                                        // Quote message
                                        if let (Some(sym), Some(bp), Some(ap), Some(bs), Some(as_size)) = (
                                            ...
                                            let _ = tx.send(MarketEvent::OrderBook(OrderBookSnapshot {
                                                symbol: sym.to_string(),
                                                bids: vec![OrderBookLevel { price: bp, qty: bs }],
                                                asks: vec![OrderBookLevel { price: ap, qty: as_size }],
                                            })).await;
                                        }
